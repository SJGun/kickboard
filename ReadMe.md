...
-  전체 구조를 생각하면서 틀을 짰으나 만들어 보니 더 효율적인 구조가 생각나서 중간에 바꿨는데 시간이 좀 걸렸다
- 에러를 생각하면서 짜는게 조금 어려웠다 특히 서버와 통신할 때 제대로 값이 오지 않을 때를 잘 고려해야 했다
- 항상 재사용성을 생각하면서 코드를 짜는 습관이 중요하다는 생각
